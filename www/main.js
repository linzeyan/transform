...
function updateCoderTexts() {
	...
	if (elements.hashControls) {
		...
		if (show) {
			if (elements.hashEncoding)
				elements.hashEncoding.value = state.hashEncoding;
			if (elements.hashUseHmac)
				elements.hashUseHmac.checked = state.hashUseHmac;
...
function encodeDigest(hexValue) {
	const lower = state.hashUppercase
...
function renderUrlQueryTable() {
	...
	if (!params.length) {
		if (elements.urlQueryEmpty)
			elements.urlQueryEmpty.classList.remove("hidden");
		return;
	}
...
function renderIPv4Results(data) {
	...
	elements.ipv4Results.innerHTML = `<div class="ipv4-stats">${stats.join(
		""
	)}</div>`;
}
...
function renderUUIDs() {
	...
	const rows = entries
		.map(([version, value]) => {
			...
			return `
        <div class="uuid-row" data-value="${escapeAttr(
					display
...
function renderCertCard(entry, list) {
	const subject = escapeHTML(
		entry.subjectCommonName || entry.subject || "Unknown subject"
	);
	...
	const eku = formatBadgeList(
		entry.extendedKeyUsage || [],
		"Extended Key Usage"
	);
    ...
    <div class="cert-card">
      <div class="cert-card-head">
        <div>
          ...
          <h3>${subject}</h3>
          <p class="muted">${
						issuerHint ? `${escapeHTML(issuerHint)} · ` : ""
					}Issuer: ${issuer}</p>
        </div>
      ...
      <div class="cert-block">
        ...
        <div class="cert-fingerprints">
          <div><label>SHA-256</label><code>${escapeHTML(
						fingerprints.sha256 || ""
					)}</code></div>
          <div><label>SHA-1</label><code>${escapeHTML(
						fingerprints.sha1 || ""
					)}</code></div>
...
function describeCertRole(entry, list) {
	const isIssuer = list?.some?.(
		(item) => item.issuerPosition === entry.position
	);
	...
	const buttons = specialCharacters
		.map((symbol) => {
			...
			return `<button type="button" data-symbol="${escapeAttr(
				symbol
			)}" data-active="${isActive ? "true" : "false"}">${escapeHTML(
				symbol
			)}</button>`;
		})
		.join("");
...
function updateRandomResults(values) {
	...
	const items = state.randomResults
		.map(
			(value) => `
        <div class="random-result-item" data-random-value="${escapeAttr(
					value
				)}">
          ${escapeHTML(value)}
        </div>
...
function handleSshKdfChange(event) {
	...
	const clamped = Math.min(
		500,
		Math.max(16, Number.isFinite(value) ? value : 16)
	);
...
function renderDataColumnCard(tableName, column) {
	...
	const controls = isNumeric
        ...
        <label>
          <span>Min</span>
          <input type="number" data-table="${escapeAttr(
						tableName
					)}" data-column="${escapeAttr(
				column.name
        ...
        <label>
          ...
          <input type="text" data-table="${escapeAttr(
						tableName
					)}" data-column="${escapeAttr(
				column.name
		...
		? `<div class="muted">Enum values: ${escapeHTML(
...
async function runBcryptHash() {
	...
	if (!password) return;
	const cost = clampNumber(
		elements.bcryptCost?.value,
		4,
		31,
		state.kdf.bcryptCost
	);
...
async function verifyBcrypt() {
	...
	if (!hash) {
		setKdfStatus(
			"bcryptStatus",
			"Please paste or generate a bcrypt hash",
			true
		);
		return;
	}
...
async function runArgonHash() {
	...
	const time = clampNumber(
		elements.argonTime?.value,
		1,
		12,
		state.kdf.argonTime
	);
	const mem = clampNumber(
		elements.argonMem?.value,
		4096,
		1048576,
		state.kdf.argonMem
	);
	...
	const hashLen = clampNumber(
		elements.argonHashLen?.value,
		8,
		128,
		state.kdf.argonHashLen
	);
	...
	try {
		...
		const encoded = await argon2_hash(
			password,
			salt,
			time,
			mem,
...
async function verifyArgon() {
	...
	if (!encoded) {
		setKdfStatus(
			"argonStatus",
			"Please paste or generate Argon2 PHC string",
			true
		);
		return;
	}
